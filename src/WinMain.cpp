...
int WINAPI WinMain( HINSTANCE instance, HINSTANCE, LPSTR, int cmdShow )
{
...
#ifdef UNICODE
	const auto *const WindowClassName = L"Cubit";
#else
	const auto *const WindowClassName = "Cubit";
#endif
	...
	HWND wnd = CreateWindowA( "Cubit", AppTitle,
	...
	i32 messageHandlingCount = 16;

	// Main message loop
	MSG msg = { 0 };
	while( WM_QUIT != msg.message )
	{
		while( --messageHandlingCount && PeekMessage( &msg, NULL, 0, 0, PM_REMOVE ) )
		{
			TranslateMessage( &msg );
			DispatchMessage ( &msg );
		}

		messageHandlingCount = 16;

		{
			auto CurrTime = Timer.GetElapsedTime();
			auto ElapsedTime = std::min( 0.05, CurrTime - PrevTime );
			PrevTime = CurrTime;
