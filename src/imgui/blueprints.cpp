...
void ShowLeftPane( float paneWidth )
{
	...
	int saveIconWidth     = 24; //Application_GetTextureWidth( s_SaveIcon );
	int saveIconHeight    = 24; //Application_GetTextureWidth( s_SaveIcon );
	int restoreIconWidth  = 24; //Application_GetTextureWidth( s_RestoreIcon );
	int restoreIconHeight = 24; //Application_GetTextureWidth( s_RestoreIcon );
...
# endif
	...
	ed::Begin( "Node editor" );
	{
		...
		//axUtil::BlueprintNodeBuilder builder( s_HeaderBackground, Application_GetTextureWidth( s_HeaderBackground ), Application_GetTextureHeight( s_HeaderBackground ) );


		//* TODO PORT Textures
		axUtil::BlueprintNodeBuilder builder(s_HeaderBackground, 64, 64);
		/*/

		axUtil::BlueprintNodeBuilder builder;
...
ImTextureID ImGui_LoadTexture( const char *path )
{
	...
	if( auto data = stbi_load( path, &width, &height, &component, 4 ) )
	{
		...
		//ImGuiDiligentRenderer::Get()->

		const auto size = ImVec2( (float)width, (float)height );

		auto texture = Diligent::ImGuiDiligentRenderer::Get()->CreateTexture(size, data);
